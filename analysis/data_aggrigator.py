...
calg = calg.pivot('time', 'address', 'margin')
...
labels = fcluster(hierarchy, 0.38, criterion='distance')
labels
...
calg.cluster = calg.cluster.replace([1, 2, 3, 4], letters)
# calg['cluster_cat'] = calg.cluster
sns.scatterplot('lat', 'lng', hue='cluster', data=calg, )


with sns.color_palette("husl", 8):
    _ = sns.scatterplot('lat', 'lng', hue='cluster', data=calg, legend=False)